...
alldir = '/path/to/ADNI3/code/HPC_Hopf_SUB_DTI_1000_Staging/results/G_range/HC_ABpos/';

saveDir = fullfile(alldir, 'optG_HC_ABpos');
...
% -------------------------------------------------
% GROUP-LEVEL mean optimal G (HC_ABpos group)
% -------------------------------------------------
mean_optG_HC = mean(optG_all_subjects);
std_optG_HC  = std(optG_all_subjects);

fprintf('\nGroup-level mean optimal G (HC_ABpos): %.4f\n', mean_optG_HC);
fprintf('Standard deviation: %.4f\n', std_optG_HC);

% Save group summary
save(fullfile(saveDir, 'optG_HC_ABpos_group_stats.mat'), ...
    'optG_all_subjects', 'mean_optG_HC', 'std_optG_HC');
