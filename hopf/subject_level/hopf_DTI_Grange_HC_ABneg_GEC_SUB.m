function hopf_DTI_Grange_HC_ABneg_GEC_SUB(nsub, s, condition)
% SUBJECT-LEVEL Hopf model (HC Aβ-, Schaefer-1000)
%
...
%   - CorrFcn_SUB_ABneg(nsub,:,:)         : subject-specific empirical corrfcn(r)
%   - Ceff from GEC_SUB_%03d_ADNI3_HC_ABneg_GEC.mat : subject-specific EC
%
% Saves:
%   WG_nsub_s_condition_ADNI3_HC_ABneg_GEC_sch1000.mat
...
%% Load subject-level f_diff and empirical corrfcn (HC Aβ−)
...
% Expected: CorrFcn_SUB_ABneg [Nneg x NPARCELLS x NR], rr [NPARCELLS x NPARCELLS], xrange [1 x NR]
load(spacorr_file, 'CorrFcn_SUB_ABneg', 'rr', 'xrange');

[NSUB_ABneg, NPARCELLS, NR] = size(CorrFcn_SUB_ABneg);
if nsub < 1 || nsub > NSUB_ABneg
    error('nsub=%d is out of range for HC Aβ− subjects (1..%d).', nsub, NSUB_ABneg);
end

% Empirical corrfcn for this subject: [NPARCELLS x NR]
empcorrfcn = squeeze(CorrFcn_SUB_ABneg(nsub,:,:));

%% Load subject-specific effective connectivity
gec_file = fullfile(code_dir,'data','GEC_SUB_ADNI3_HC_ABneg',sprintf('GEC_SUB_%03d_ADNI3_HC_ABneg_GEC.mat', nsub));
...
Tmax  = 197;
f_sub = squeeze(f_diff_SUB_ABneg(nsub,:));   % ensure vector [NPARCELLS x 1]
...
%% Save results
out_name = fullfile(out_dir,'G_range','HC_ABneg',sprintf('WG_%03d_%03d_%d_ADNI3_HC_ABneg_GEC_sch1000.mat', ...
                   nsub, s, condition));
