...
alldir = '/path/to/ADNI3/code/HPC_Hopf_SUB_DTI_1000_Staging/results/G_range/MCI_ABpos/';

saveDir = fullfile(alldir, 'optG_MCI_ABpos');
...
% -------------------------------------------------
% GROUP-LEVEL mean optimal G (MCI_ABpos group)
% -------------------------------------------------
mean_optG_MCI = mean(optG_all_subjects);
std_optG_MCI  = std(optG_all_subjects);

fprintf('\nGroup-level mean optimal G (MCI_ABpos): %.4f\n', mean_optG_MCI);
fprintf('Standard deviation: %.4f\n', std_optG_MCI);

% Save group summary
save(fullfile(saveDir, 'optG_MCI_ABpos_group_stats.mat'), ...
    'optG_all_subjects', 'mean_optG_MCI', 'std_optG_MCI');
