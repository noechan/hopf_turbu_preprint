function pert_infocapacity_susc_HC_ABpos_GEC_SUB(nsub, s, condition)
...
%   - Ceff from GEC_SUB_%03d_ADNI3_HC_ABpos_GEC.mat : subject-specific EC
%   - optG_HC_ABpos(nsub)                 : subject-specific optimal global coupling
%
% Saves:
%   Wtrials_s_condition_err_hete_ADNI3_HC_ABpos_GEC_sch1000_SUB%03d.mat
%
...
out_dir = fullfile(code_dir, 'results','Wtrials','HC_ABpos');
...
freq_file = fullfile(code_dir,'data','results_f_diff_fce_cond1_ADNI3_HC_ABpos_sch1000_SUB.mat');
...
if nsub < 1 || nsub > N_SUB_ABpos
    error('nsub=%d is out of range for HC Aβ+ subjects (1..%d).', nsub, N_SUB_ABpos);
end
...
spacorr_file = fullfile(code_dir,'data','empirical_spacorr_rest_cond_1_ADNI3_HC_ABpos_sch1000_SUB.mat');
...
gec_file = fullfile(code_dir,'data','GEC_SUB_ADNI3_HC_ABpos', ...
                    sprintf('GEC_SUB_%03d_ADNI3_HC_ABpos_GEC.mat', nsub));
load(gec_file, 'Ceff');   % [NPARCELLS x NPARCELLS]

%% Global coupling G (subject-specific optimal G)
if condition == 1
    optG_file = fullfile(code_dir,'results','G_range','HC_ABpos','optG_HC_ABpos', ...
                         'optG_HC_ABpos_group_stats.mat');
    ...
    optG_HC_ABpos = optG_all_subjects(:);   % ensure column vector

    if numel(optG_HC_ABpos) < nsub
        error('optG_HC_ABpos has length %d, but nsub=%d.', numel(optG_HC_ABpos), nsub);
    end

    G = optG_HC_ABpos(nsub);
else
...
out_name = fullfile(out_dir,...
    sprintf('Wtrials_%03d_%d_%s_ADNI3_HC_ABpos_GEC_sch1000_SUB%03d.mat', ...
