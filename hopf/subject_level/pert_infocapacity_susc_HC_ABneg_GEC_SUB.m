function pert_infocapacity_susc_HC_ABneg_GEC_SUB(nsub, s, condition)
% SUBJECT-LEVEL infocapacity/susceptibility in the Hopf model (HC Aβ-, Schaefer-1000)
%
% nsub      : subject index within HC Aβ- subgroup (1..Nneg)
% s         : random seed index (integer, used for rng and filenames)
% condition : kept for compatibility (typically 1)
%
% Uses:
%   - f_diff_SUB_ABneg(nsub,:)            : subject-specific dominant frequency
%   - rr, xrange                          : distance bins (from *_SUB* spacorr file)
%   - Ceff from GEC_SUB_%03d_ADNI3_HC_ABneg_GEC.mat : subject-specific EC
%   - optG_HC_ABneg(nsub)                 : subject-specific optimal global coupling
%
% Saves:
%   Wtrials_s_condition_err_hete_ADNI3_HC_ABneg_GEC_sch1000_SUB%03d.mat
%
...
out_dir = fullfile(code_dir, 'results','Wtrials','HC_ABneg');
...
%% Load subject-level f_diff (HC Aβ-)
freq_file = fullfile(code_dir,'data','results_f_diff_fce_cond1_ADNI3_HC_ABneg_sch1000_SUB.mat');
% Expected: f_diff_SUB_ABneg [Nneg x NPARCELLS]
load(freq_file, 'f_diff_SUB_ABneg');

[N_SUB_ABneg, NPARCELLS] = size(f_diff_SUB_ABneg);
if nsub < 1 || nsub > N_SUB_ABneg
    error('nsub=%d is out of range for HC Aβ- subjects (1..%d).', nsub, N_SUB_ABneg);
end

f_sub = squeeze(f_diff_SUB_ABneg(nsub,:));  % [1 x NPARCELLS] or [NPARCELLS x 1]

%% Load distance matrix rr and xrange (from subject-level spacorr file)
spacorr_file = fullfile(code_dir,'data','empirical_spacorr_rest_cond_1_ADNI3_HC_ABneg_sch1000_SUB.mat');
...
gec_file = fullfile(code_dir,'data','GEC_SUB_ADNI3_HC_ABneg', ...
                    sprintf('GEC_SUB_%03d_ADNI3_HC_ABneg_GEC.mat', nsub));
load(gec_file, 'Ceff');   % [NPARCELLS x NPARCELLS]

%% Global coupling G (subject-specific optimal G)
if condition == 1
    optG_file = fullfile(code_dir,'results','G_range','HC_ABneg','optG_HC_ABneg', ...
                         'optG_HC_ABneg_group_stats.mat');
    % This file contains: optG_all_subjects, mean_optG_HC, std_optG_HC
    load(optG_file, 'optG_all_subjects');

    % rename to what this function expects
    optG_HC_ABneg = optG_all_subjects(:);   % ensure column vector

    if numel(optG_HC_ABneg) < nsub
        error('optG_HC_ABneg has length %d, but nsub=%d.', numel(optG_HC_ABneg), nsub);
    end

    G = optG_HC_ABneg(nsub);
else
...
out_name = fullfile(out_dir,...
    sprintf('Wtrials_%03d_%d_%s_ADNI3_HC_ABneg_GEC_sch1000_SUB%03d.mat', ...
