function hopf_DTI_Grange_AD_ABpos_GEC_SUB(nsub, s, condition)
% SUBJECT-LEVEL Hopf model (AD Aβ−, Schaefer-1000)
%
% nsub      : subject index within AD Aβ− subgroup (1..Nneg)
...
%
% Saves:
%   WG_nsub_s_condition_ADNI3_AD_ABpos_GEC_sch1000.mat
...
%% Load subject-level f_diff and empirical corrfcn (AD Aβ−)
...
if nsub < 1 || nsub > NSUB_ABpos
    error('nsub=%d is out of range for AD Aβ+ subjects (1..%d).', nsub, NSUB_ABpos);
end
...
gec_file = fullfile(code_dir,'data','GEC_SUB_ADNI3_AD_ABpos',sprintf('GEC_SUB_%03d_ADNI3_AD_ABpos_GEC.mat', nsub));
...
fprintf('AD Aβ+ subject nsub=%d | G index s=%d | G=%.2f\n', nsub, s, G);
...
%% Save results
out_name = fullfile(out_dir,'G_range','AD_ABpos',sprintf('WG_%03d_%03d_%d_ADNI3_AD_ABpos_GEC_sch1000.mat', ...
                   nsub, s, condition));
