...
load('tseries_ADNI3_AD_MPRAGE_IRFSPGR_sch1000_N238rev.mat')              % Denoised time-series           % Denoised time-series
...
load(fullfile(ptid_path, 'PTID_ADNI3_AD_MPRAGE_IRFSPGR_all.mat'))     % Loads variable: PTID
PTID_AD = PTID;
...
% fce by AB status
fce_AD_ABpos=squeeze(mean(fce1(isAbetaPos_AD,:,:)));
fce_AD_ABneg=squeeze(mean(fce1(isAbetaNeg_AD,:,:)));
% freq by AB status
 Power_Areas_AD_ABpos=squeeze(mean(PowSpect(:,:,isAbetaPos_AD),3)); %power for each freq averaged for all subjects
    for seed=1:NPARCELLS
        Power_Areas_AD_ABpos(:,seed)=gaussfilt(freq,Power_Areas_AD_ABpos(:,seed)',0.01);
    end
    
    [maxpowdata_AD_ABpos,index_AD_ABpos]=max(Power_Areas_AD_ABpos);
    f_diff_AD_ABpos = freq(index_AD_ABpos);
    f_diff_AD_ABpos(find(f_diff_AD_ABpos==0))=mean(f_diff_AD_ABpos(find(f_diff_AD_ABpos~=0)));

     Power_Areas_AD_ABneg=squeeze(mean(PowSpect(:,:,isAbetaNeg_AD),3)); %power for each freq averaged for all subjects
    for seed=1:NPARCELLS
        Power_Areas_AD_ABneg(:,seed)=gaussfilt(freq,Power_Areas_AD_ABneg(:,seed)',0.01);
    end
    
    [maxpowdata_AD_ABneg,index_AD_ABneg]=max(Power_Areas_AD_ABneg);
    f_diff_AD_ABneg = freq(index_AD_ABneg);
    f_diff_AD_ABneg(find(f_diff_AD_ABneg==0))=mean(f_diff_AD_ABneg(find(f_diff_AD_ABneg~=0)));

    
    cd('/path/to/ADNI3/code/HPC_Hopf_DTI_1000_Staging/')
    save (sprintf('results_f_diff_fce_cond%d_ADNI3_AD_sch1000.mat', cond),...
    'f_diff', 'fce');
    save (sprintf('results_f_diff_fce_cond%d_ADNI3_AD_ABpos_sch1000.mat', cond),...
    'f_diff_AD_ABpos', 'fce_AD_ABpos');
    save (sprintf('results_f_diff_fce_cond%d_ADNI3_AD_ABneg_sch1000.mat', cond),...
    'f_diff_AD_ABneg', 'fce_AD_ABneg');
end
