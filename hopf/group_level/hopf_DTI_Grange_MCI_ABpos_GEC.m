...
function hopf_DTI_Grange_MCI_ABpos(s,condition)

%load data
load results_f_diff_fce_cond1_ADNI3_MCI_ABpos_sch1000.mat; % load frequency with maximal power (of narrowly filtered data) by area computed in script Compute_Hopf_Freq_mci.m
load empirical_spacorr_rest_cond_1_ADNI3_MCI_ABpos_sch1000.mat; %load empirical FC computed in script Empirical_corrfcn_mci.m
load ADNI3_MCI_ABpos_GECglo_cnd_001_sch1000_mod_eps_1_2_ratio_SC_max02.mat; %load generative effective connectivity 
load SchaeferCOG.mat; %structural connectivity template from DTI provided by Morten L. Kringelbach
load tseries_ADNI3_MCI_MPRAGE_IRFSPGR_sch1000_N238rev.mat; %load empirical time-series
...
%% Load PTIDs
load 'PTID_ADNI3_MCI_MPRAGE_IRFSPGR_all.mat'     % Loads variable: PTID
PTID_MCI = PTID;
...
tseries_MCI_ABpos=tseries(isAbetaPos_MCI,:);


%Parameters and G range
xs=tseries_MCI_ABpos(:,condition);
...
%% Parameters HOPF
Tmax=197;
omega = repmat(2*pi*f_diff_MCI_ABpos',1,2); omega(:,1) = -omega(:,1); %angular velocity
...
save(sprintf('WG_%03d_%d_ADNI3_MCI_ABpos_GEC_sch1000.mat',s,condition),'InfoCascade','InfoFlow','Turbulence','err_hete');
