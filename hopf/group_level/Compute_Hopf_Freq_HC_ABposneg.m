...
% fce by AB status
fce_HC_ABpos=squeeze(mean(fce1(isAbetaPos_HC,:,:)));
fce_HC_ABneg=squeeze(mean(fce1(isAbetaNeg_HC,:,:)));
% freq by AB status
 Power_Areas_HC_ABpos=squeeze(mean(PowSpect(:,:,isAbetaPos_HC),3)); %power for each freq averaged for all subjects
    for seed=1:NPARCELLS
        Power_Areas_HC_ABpos(:,seed)=gaussfilt(freq,Power_Areas_HC_ABpos(:,seed)',0.01);
    end
    
    [maxpowdata_HC_ABpos,index_HC_ABpos]=max(Power_Areas_HC_ABpos);
    f_diff_HC_ABpos = freq(index_HC_ABpos);
    f_diff_HC_ABpos(find(f_diff_HC_ABpos==0))=mean(f_diff_HC_ABpos(find(f_diff_HC_ABpos~=0)));

     Power_Areas_HC_ABneg=squeeze(mean(PowSpect(:,:,isAbetaNeg_HC),3)); %power for each freq averaged for all subjects
    for seed=1:NPARCELLS
        Power_Areas_HC_ABneg(:,seed)=gaussfilt(freq,Power_Areas_HC_ABneg(:,seed)',0.01);
    end
    
    [maxpowdata_HC_ABneg,index_HC_ABneg]=max(Power_Areas_HC_ABneg);
    f_diff_HC_ABneg = freq(index_HC_ABneg);
    f_diff_HC_ABneg(find(f_diff_HC_ABneg==0))=mean(f_diff_HC_ABneg(find(f_diff_HC_ABneg~=0)));

    
    cd('/path/to/ADNI3/code/HPC_Hopf_DTI_1000_Staging/')
    save (sprintf('results_f_diff_fce_cond%d_ADNI3_HC_sch1000.mat', cond),...
    'f_diff', 'fce');
    save (sprintf('results_f_diff_fce_cond%d_ADNI3_HC_ABpos_sch1000.mat', cond),...
    'f_diff_HC_ABpos', 'fce_HC_ABpos');
    save (sprintf('results_f_diff_fce_cond%d_ADNI3_HC_ABneg_sch1000.mat', cond),...
    'f_diff_HC_ABneg', 'fce_HC_ABneg');
end
