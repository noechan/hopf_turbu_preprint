...
CorrFcn_sub_AD_ABpos = corrfcnsub(isAbetaPos_AD,:,:);
CorrFcn_sub_AD_ABneg = corrfcnsub(isAbetaNeg_AD,:,:);

% Group
    
    corrfcn=squeeze(nanmean(corrfcnsub));
    corrfcn_AD_ABpos = squeeze(nanmean(CorrFcn_sub_AD_ABpos));
    corrfcn_AD_ABneg = squeeze(nanmean(CorrFcn_sub_AD_ABneg));

    
    cd('/path/to/ADNI3/code/HPC_Hopf_DTI_1000_Staging/')
    save (sprintf('empirical_spacorr_rest_cond_%d_ADNI3_AD_sch1000.mat', cond),'corrfcn', 'corrfcnsub');
    save (sprintf('empirical_spacorr_rest_cond_%d_ADNI3_AD_ABpos_sch1000.mat', cond),'corrfcn_AD_ABpos');
    save (sprintf('empirical_spacorr_rest_cond_%d_ADNI3_AD_ABneg_sch1000.mat', cond),'corrfcn_AD_ABneg');
end