...
CorrFcn_sub_MCI_ABpos = corrfcnsub(isAbetaPos_MCI,:,:);
CorrFcn_sub_MCI_ABneg = corrfcnsub(isAbetaNeg_MCI,:,:);

% Group
    
    corrfcn=squeeze(nanmean(corrfcnsub));
    corrfcn_MCI_ABpos = squeeze(nanmean(CorrFcn_sub_MCI_ABpos));
    corrfcn_MCI_ABneg = squeeze(nanmean(CorrFcn_sub_MCI_ABneg));

    
    cd('/path/to/ADNI3/code/HPC_Hopf_DTI_1000_Staging/')
    save (sprintf('empirical_spacorr_rest_cond_%d_ADNI3_MCI_sch1000.mat', cond),'corrfcn', 'corrfcnsub');
    save (sprintf('empirical_spacorr_rest_cond_%d_ADNI3_MCI_ABpos_sch1000.mat', cond),'corrfcn_MCI_ABpos');
    save (sprintf('empirical_spacorr_rest_cond_%d_ADNI3_MCI_ABneg_sch1000.mat', cond),'corrfcn_MCI_ABneg');
end