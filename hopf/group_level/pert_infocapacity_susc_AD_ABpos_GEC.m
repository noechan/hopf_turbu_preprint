function infocapacity_hopf_errorhete_AD_ABpos(s,condition)
...
load 'results_f_diff_fce_cond1_ADNI3_AD_ABpos_sch1000.mat'; % load frequency with maximal power (of narrowly filtered data) by area computed in script Compute_Hopf_Freq_ad.m
load 'empirical_spacorr_rest_cond_1_ADNI3_AD_ABpos_sch1000.mat'; %load empirical FC computed in script Empirical_corrfcn_ad.m
...
%% Parameters HOPF
Tmax=197;
omega = repmat(2*pi*f_diff_AD_ABpos',1,2); omega(:,1) = -omega(:,1);
dt=0.1*TR/2;
sig=0.01;
dsig = sqrt(dt)*sig;
...
save(sprintf('Wtrials_%03d_%d_%s_ADNI3_AD_ABpos_GEC_sch1000.mat',s,condition,measure),'infocapacity','susceptibility');
