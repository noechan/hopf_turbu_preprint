...
output_dir = Path(
    os.environ.get(
        "AHBA_GENE_NIFTI_DIR_SCH400",
        script_dir / "gene_niftis" / "schaefer400_2mm",
    )
).expanduser().resolve()
...
# Extract gene expression mapped to the Schaefer400 atlas
...
expression_data.to_csv(script_dir / "full_GE_schaefer400_2mm.csv")
gene_names = expression_data.columns.tolist()
pd.Series(gene_names).to_csv(
    script_dir / "full_gene_symbols_schaefer400_2mm.csv",
    index=False,
    header=["GeneSymbol"],
)
print(type(report))
with (script_dir / "abagen_report_schaefer400_2mm.txt").open(
    "w", encoding="utf-8"
) as f:
    f.write(report)
...
gene_expression_data.to_csv(script_dir / "gene_expression_AD_schaefer400_2mm.csv")
...
def create_gene_nifti(gene_name, atlas_data, atlas_img):
    ...
    for roi_idx in range(1, 401):  # Schaefer100 has 400 ROIs, indexed 1-400
    ...
    output_path = output_dir / f"{gene_name}_expression_sch400_2mm.nii.gz"
