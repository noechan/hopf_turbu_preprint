%% Create timeseries for turbulence across multiple parcellations: HC_MPRAGE
...
batch_path = { ...
    'ADNI3_HC_MPRAGE_batch_1_extended', ...
    'ADNI3_HC_MPRAGE_batch_2_extended', ...
    'ADNI3_HC_MPRAGE_batch_3_extended'};

tseries_path = { ...
    'ADNI3_HC_MPRAGE_batch_1_N238rev', ...
    'ADNI3_HC_MPRAGE_batch_2_N238rev', ...
    'ADNI3_HC_MPRAGE_batch_3_N238rev'};
...
% CONN data index where Schaefer400 starts for all batches
first_roi_idx = 168;

% Parcellations in CONN output order:
% Schaefer400 -> Schaefer1000 -> Schaefer100 -> DBS80 -> Glasser360
...
for batch = 1:numel(batch_path)
    ...
    for p = 1:numel(parcellations)
        ...
        for s = 1:numel(fnames_matching)
            ...
            if numel(data) < end_idx
                error(['Subject %d file has only %d data entries, but %s requires index %d. ', ...
                       'Check ROI ordering or CONN output structure.'], ...
        ...
        % Save one MAT file per parcellation
        save_filename = sprintf( ...
            'tseries_ADNI3_HC_MPRAGE_batch%d_%s_matching_QC.mat', ...
            batch, parc_name);
