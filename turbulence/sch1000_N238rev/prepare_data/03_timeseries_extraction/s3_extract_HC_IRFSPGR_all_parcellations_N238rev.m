%% Create timeseries for turbulence across multiple parcellations
...
batch_path   = {'ADNI3_HC_IRFSPGR_batch_1_extended'};
tseries_path = {'ADNI3_HC_IRFSPGR_batch_1_N238rev'};
...
% CONN data index where Schaefer400 starts
first_roi_idx = 168;
...
for batch = 1:numel(batch_path)
    ...
    % Load CONN IDs
    connIDs_varname  = sprintf('connIDs_BIDS_IRFSPGR_60_89_batch_%d_HC', batch);
    ...
    for p = 1:numel(parcellations)
        ...
        for s = 1:numel(fnames_matching)
            ...
            % Safety check: make sure the required atlas indices exist
        ...
        save_filename = sprintf( ...
            'tseries_ADNI3_HC_IRFSPGR_batch%d_%s_matching_QC.mat', ...
            batch, parc_name);
