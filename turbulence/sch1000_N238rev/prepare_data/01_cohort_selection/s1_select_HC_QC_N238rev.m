...
HC_data_path = P.participants_cross_sectional;
code_path = P.prepare_metadata;
if ~isfolder(code_path), mkdir(code_path); end
% Specify the file name
filename = 'ADNI3_HC_ALL_MRI_sc_analysis_N238rev_idx4MRIselection.xlsx';
sheet = 'HC_removed_N238rev_idx'; % Specify the sheet name
% Read the data from the Excel file
data = readtable(fullfile(HC_data_path,filename), 'Sheet', sheet);
...
for i = 1:length(unique_batches)
    ...
    connIDs_field = ['connIDs_', strrep(batch_name, ' ', '_'),'_HC']; % Replace spaces with underscores
    ptid_field = ['PTID_', strrep(batch_name, ' ', '_'),'_HC']; % Replace spaces with underscores
