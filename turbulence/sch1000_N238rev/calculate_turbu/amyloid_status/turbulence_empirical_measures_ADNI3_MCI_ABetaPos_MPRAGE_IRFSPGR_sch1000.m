%% Turbulence observables: MCI Aβ-positive group
clearvars; close all
...
input_file = fullfile(P.input.MCI, ...
    'tseries_ADNI3_MCI_ABetaPos_MPRAGE_IRFSPGR_sch1000_N238rev.mat');
input_data = load(input_file, 'tseries_MCI_ABpos', 'PTID_MCI_ABpos');
xs = input_data.tseries_MCI_ABpos(:);
PTID = cellstr(string(input_data.PTID_MCI_ABpos(:)));
group_key = 'MCI_ABetaPos';
group_label = 'MCI Aβ+';
output_dir = P.output.MCI_ABetaPos;
