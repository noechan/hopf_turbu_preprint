%% Turbulence observables: AD Aβ-positive group
clearvars; close all
...
input_file = fullfile(P.input.AD, ...
    'tseries_ADNI3_AD_ABetaPos_MPRAGE_IRFSPGR_sch1000_N238rev.mat');
input_data = load(input_file, 'tseries_AD_ABpos', 'PTID_AD_ABpos');
xs = input_data.tseries_AD_ABpos(:);
PTID = cellstr(string(input_data.PTID_AD_ABpos(:)));
group_key = 'AD_ABetaPos';
group_label = 'AD Aβ+';
output_dir = P.output.AD_ABetaPos;
