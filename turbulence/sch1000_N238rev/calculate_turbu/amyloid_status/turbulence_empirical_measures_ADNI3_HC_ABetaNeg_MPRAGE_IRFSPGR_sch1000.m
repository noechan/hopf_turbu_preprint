%% Turbulence observables: cognitively healthy Aβ-negative group
clearvars; close all
...
input_file = fullfile(P.input.HC, ...
    'tseries_ADNI3_HC_ABetaNeg_MPRAGE_IRFSPGR_sch1000_N238rev.mat');
input_data = load(input_file, 'tseries_HC_ABneg', 'PTID_HC_ABneg');
xs = input_data.tseries_HC_ABneg(:);
PTID = cellstr(string(input_data.PTID_HC_ABneg(:)));
group_key = 'HC_ABetaNeg';
group_label = 'HC Aβ−';
output_dir = P.output.HC_ABetaNeg;
